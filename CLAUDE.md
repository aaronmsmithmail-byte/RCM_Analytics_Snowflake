...
pytest tests/ -q | tail -1  # shows the count — currently 362 passed
```
...
- `tests/test_app_utils.py` — 31 tests covering `df_to_csv`, `dfs_to_excel`, `_linear_forecast`, `_forecast_model_stats`, `_detect_anomalies`, `_detect_seasonality` from `app.py`
