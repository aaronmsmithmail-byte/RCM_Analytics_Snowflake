...
def _cte(p: FilterParams):
    ...
    clauses = [f"c.DATE_OF_SERVICE >= '{_esc(p.start_date)}'",
               f"c.DATE_OF_SERVICE <= '{_esc(p.end_date)}'"]
    if p.payer_id:
...
"""
    ...
    df["ncr"] = np.where((df["charges"] - df["contractual_adj"]) > 0,
                         df["payments"] / (df["charges"] - df["contractual_adj"]) * 100, 0)
    df = _set_period_index(df)
...
"""
    ...
    df["recovery_rate"] = np.where(df["total_denied_amount"] > 0,
                                   df["total_recovered"] / df["total_denied_amount"] * 100, 0)
    return df
...
"""
    ...
    trend["cost_to_collect_pct"] = np.where(trend["collections"] > 0,
                                            trend["rcm_cost"] / trend["collections"] * 100, 0)
...
def query_ar_aging(p: FilterParams):
    empty_summary = pd.DataFrame({"claim_count": 0, "total_ar": 0.0, "pct_of_total": 0.0},
...
"""
    ...
    summary = (df.groupby("aging_bucket")
...
"""
    ...
    df["collection_rate"] = np.where(df["total_charges"] > 0, df["total_payments"] / df["total_charges"] * 100, 0)
    df["avg_payment_per_encounter"] = np.where(df["encounter_count"] > 0,
                                               df["total_payments"] / df["encounter_count"], 0)
    return df
...
"""
    df = _query(sql)
    if df.empty:
        return pd.DataFrame(columns=["provider_id", "provider_name", "specialty", "department",
...
"""
    df = _query(sql)
    if df.empty:
        return pd.DataFrame(columns=["cpt_code", "cpt_description", "charge_count", "total_units",
                                     "total_charges", "claim_count", "denied_claims",
...
"""
    df = _query(sql)
    empty_cols = ["payer_id", "payer_name", "payer_type", "payment_count",
...
"""
    df = _query(sql)
    if df.empty:
        return pd.DataFrame(columns=["payer_name", "payer_type", "payment_count",
...
"""
    df = _query(sql)
    if df.empty:
        return pd.DataFrame(columns=["department", "encounter_type", "claim_count",
...
"""
    ...
    df["patient_resp_rate"] = np.where(df["total_allowed"] > 0,
                                       df["total_patient_resp"] / df["total_allowed"] * 100, 0)
    df = df.set_index("period")
    df.index.name = "year_month"
    return df
...
_DOMAIN_CADENCE_HOURS = {
    "claims": 4, "payments": 6, "encounters": 4, "charges": 4,
    "denials": 12, "adjustments": 8, "payers": 24, "patients": 24,
    "providers": 24, "operating_costs": 720,
}

_DOMAIN_LABELS = {
    "claims": "Claims", "payments": "Payments / ERA", "encounters": "Encounters / ADT",
    "charges": "Charges / CDM", "denials": "Denials", "adjustments": "Adjustments",
    "payers": "Payer Master", "patients": "Patient Demographics",
    "providers": "Provider Roster", "operating_costs": "Operating Costs",
}
...
def query_data_freshness():
    ...
    df = _query(sql)
    if df.empty:
        return pd.DataFrame(columns=["domain", "label", "last_loaded_at", "row_count",
