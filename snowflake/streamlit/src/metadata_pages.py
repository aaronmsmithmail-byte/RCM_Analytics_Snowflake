...
# RCM_ANALYTICS.METADATA.KPI_CATALOG from Snowflake at render time; this list is used only
...
# Keyed by system NAME (from RCM_ANALYTICS.METADATA.KG_NODES.source_system) — the name itself
...
# ── Semantic Layer data ───────────────────────────────────────────────
# NOTE: render_semantic_layer() queries RCM_ANALYTICS.METADATA.SEMANTIC_LAYER from Snowflake.
...
def render_data_lineage():
    ...
    _ss_df = _query_meta(
        "SELECT entity_id, COALESCE(source_system, '') AS source_system FROM RCM_ANALYTICS.METADATA.KG_NODES"
    )
    ...
    with dot.subgraph(name="cluster_services") as c:
        ...
        c.node("cube_svc", "Cortex Analyst\nSemantic Model", shape="box3d", fillcolor="#ede9fe", color="#9561e2")
        c.node("neo4j_svc", "Snowflake\nMETADATA Schema", shape="cylinder", fillcolor="#dcfce7", color="#38c172")
...
def render_knowledge_graph():
    """Interactive entity-relationship diagram — sourced from Snowflake METADATA schema."""
    st.title("Knowledge Graph")
    st.caption(
        "Entity relationships across the 10 data tables (11 foreign-key relationships). Powered by Snowflake METADATA tables."
    )

    # ── Query metadata from Snowflake ─────────────────────────────────
    nodes_meta = _query_meta("SELECT entity_id, silver_table, description FROM RCM_ANALYTICS.METADATA.KG_NODES")
    desc_map = {}
    if not nodes_meta.empty:
        nodes_meta.columns = [c.lower() for c in nodes_meta.columns]
        desc_map = {row.entity_id: row.description for _, row in nodes_meta.iterrows()}

    edges_meta = _query_meta(
        "SELECT parent_entity, child_entity, join_column, cardinality FROM RCM_ANALYTICS.METADATA.KG_EDGES"
    )
    if not edges_meta.empty:
        edges_meta.columns = [c.lower() for c in edges_meta.columns]
        live_edges = [
            {
...
def render_semantic_layer():
    """Business concept → KPI → raw column mapping. Powered by Snowflake METADATA + Cortex Analyst."""
    st.title("Semantic Layer")
    st.caption("How business questions map to KPIs and raw data columns. Powered by Cortex Analyst semantic model.")
...
def render_data_validation(issues: list[dict]):
    ...
    st.dataframe(checks_ref, use_container_width=True)
...
def _fetch_process_kpis() -> dict:
    ...
    return {
        ...
        "clean_claim_rate": _val(
            ...
            "/ NULLIF(COUNT(*), 0), 1) FROM RCM_ANALYTICS.SILVER.CLAIMS"
        ),
        "total_claims": _val("SELECT COUNT(*) FROM RCM_ANALYTICS.SILVER.CLAIMS"),
        ...
        "underpayment_rate": _val(
            ...
            "    THEN 1 ELSE 0 END) "
            "/ NULLIF(COUNT(*), 0), 1) "
            "FROM RCM_ANALYTICS.SILVER.PAYMENTS WHERE ALLOWED_AMOUNT > 0"
        ),
    }
