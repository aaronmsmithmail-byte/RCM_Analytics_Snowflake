...
GROUP BY TO_CHAR(TRY_TO_DATE(c.DATE_OF_SERVICE, 'YYYY-MM-DD'), 'YYYY-MM')
ORDER BY PERIOD;
...
SELECT
    CASE
        WHEN DATEDIFF('day', TRY_TO_DATE(c.DATE_OF_SERVICE, 'YYYY-MM-DD'), CURRENT_DATE()) <=  30 THEN '0-30 days'
        WHEN DATEDIFF('day', TRY_TO_DATE(c.DATE_OF_SERVICE, 'YYYY-MM-DD'), CURRENT_DATE()) <=  60 THEN '31-60 days'
        WHEN DATEDIFF('day', TRY_TO_DATE(c.DATE_OF_SERVICE, 'YYYY-MM-DD'), CURRENT_DATE()) <=  90 THEN '61-90 days'
        WHEN DATEDIFF('day', TRY_TO_DATE(c.DATE_OF_SERVICE, 'YYYY-MM-DD'), CURRENT_DATE()) <= 120 THEN '91-120 days'
        ELSE '120+ days'
    END                                                               AS AGING_BUCKET,
